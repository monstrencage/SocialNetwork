...
let handle : type a b. (a -> (b list)) -> a -> (b list) = fun f x ->
  try f x
  with Empty -> []
...
let rec to_string : type a b. a -> (a,b) req -> string*b Json.obj = fun x r->
  ...
  | Contacts -> (f_id x)^"/friends?",Json.F
  | MutualF(y) -> (f_id y)^"/mutualfriends/"^(f_id x)^"?",Json.F
  | Posts -> (f_id x)^"/posts?limit=1000&",Json.M
  | Feed -> (f_id x)^"/feed?limit=1000&",Json.M
  | Likes -> (f_id x)^"/likes?",Json.L
  | Infos -> (f_id x)^"?fields=name,birthday,gender&",Json.I
  | Since (d,r) -> 
